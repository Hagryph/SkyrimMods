...
// Resolve an offset token to a runtime address: an RVA off 0x140000000 (default), or a raw
// runtime VA if prefixed "abs:".
...
// Parse a value for 'write' per type into raw bits[ti.size]. Ints: hex or (signed) decimal; floats: decimal.
...
// Responses are one line each, so fold multi-line console text onto a single line: newline -> US
// (0x1f). Clients split on 0x1f to recover the lines. Trailing separators/spaces are trimmed.
std::string OneLine(const std::string& s) {
    std::string o; o.reserve(s.size());
    for (char c : s) o += (c == '\n') ? '\x1f' : c;
    while (!o.empty() && (o.back() == '\x1f' || o.back() == ' ' || o.back() == '\r')) o.pop_back();
    return o;
}
...
void Server::Stop() {
    m_running = false;
    if (m_thread.joinable()) m_thread.detach();  // it's blocked on accept(); let the process tear it down
}

void Server::Run() {
    WSADATA wsa{};
    if (::WSAStartup(MAKEWORD(2, 2), &wsa) != 0) { HAG_ERR("WSAStartup failed"); return; }

    SOCKET listenSock = ::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (listenSock == INVALID_SOCKET) { HAG_ERR("socket() failed"); ::WSACleanup(); return; }
    ...
    if (::bind(listenSock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR ||
        ...
        HAG_ERR("bind/listen failed on 127.0.0.1:{} (err {})", m_port, ::WSAGetLastError());
        ::closesocket(listenSock); ::WSACleanup(); return;
    }
    HAG_INFO("HagIPC listening on 127.0.0.1:{}", m_port);
...
std::string Server::Dispatch(const std::string& line) {
    ...
    if (cmd == "help")
        return "ok cmds: ping | base | read <off> <type> [chain..] | readb <off> <len> [chain..] | "
               ...
               "console <text> | log [bytes] | saves | load <name>  "
               "(off=RVA off 0x140000000 or abs:<VA>; type=u8/u16/u32/u64/i*/f32/f64/ptr; chain: p=*p+c each; "
               "call/exec args are 64-bit ints; only console runs on the game main thread (all other "
               "verbs run inline, so they work from the main menu); console returns the command's "
               "output and log dumps recent console text, newlines as 0x1f)";
    ...
    if (cmd == "write") {
        ...
        // Runs inline (off-thread). Only `console` is gated to the main thread.
        std::uintptr_t fa = 0;
    ...
    if (cmd == "console") {
        // Everything after the first token is the raw command line (preserve internal spaces).
        std::size_t s = line.find_first_not_of(" \t");
        std::size_t e = (s == std::string::npos) ? std::string::npos : line.find_first_of(" \t", s);
        std::size_t p = (e == std::string::npos) ? std::string::npos : line.find_first_not_of(" \t", e);
        if (p == std::string::npos) return "err usage: console <command text>";
        std::string text = line.substr(p);

        auto job = std::make_shared<console::Result>();
        auto txt = std::make_shared<std::string>(text);
        // console execution is unsafe off the main thread (form registration + script VM), so it
        // must run there or not at all.
        const bool ran = mt::RunOnMain([job, txt] { *job = console::Run(*txt); });
        if (!ran)      return "err console needs the game in a save and focused (main thread only)";
        if (job->faulted)    return "err console faulted (seh)";
        if (job->noCompiler) return "err console compiler not ready (open the in-game console once)";
        if (job->compiled) {
            std::string resp = "ok ran: " + text + "  (compiled " + std::to_string(job->compiledSize) + "b)";
            if (!job->output.empty()) resp += std::string("\x1f") + OneLine(job->output);  // command's echo/results
            return resp;
        }
        return "err did not compile: " + text + "  (unknown command?)";
    }

    if (cmd == "saves") {
        // Reads the game's OWN resolved save directory (MO2 -> ...\__MO_Saves\), then enumerates it.
        auto list = saveload::ListSaves();
        if (list.empty()) return "ok (no saves found)";
        std::string out = "ok " + std::to_string(list.size());
        for (auto& s : list) out += std::string("\x1f") + s;  // one save per 0x1f-delimited field
        return out;
    }
    ...
    if (cmd == "load") {
        ...
        std::size_t e = (s == std::string::npos) ? std::string::npos : line.find_first_of(" \t", s);
        std::size_t p = (e == std::string::npos) ? std::string::npos : line.find_first_not_of(" \t", e);
        if (p == std::string::npos) return "err usage: load <save name>  (base name, no .ess)";
        std::string name = line.substr(p);

        // Runs inline so it works from the main menu (the game queues the load request itself).
        if (!saveload::Load(name))
            return "err load failed (save-load manager not ready, or bad name): " + name;
        return "ok loading: " + name;
    }

    if (cmd == "log") {
        // Dump the tail of the console output buffer (console+0x408). Read-only, so it works even
        // when the game is unfocused; no main-thread marshaling needed.
        std::uint64_t maxBytes = 2048;
        if (tk.size() >= 2) { std::uint64_t v = 0; if (ParseU64(tk[1], v) && v) maxBytes = v; }
        if (maxBytes > 8192) maxBytes = 8192;

        const std::uintptr_t g = offsets::FromRVA(offsets::kConsolePtr);
        std::uintptr_t fa = 0;
        std::uint16_t len = 0;
        std::uintptr_t offLen[1] = { offsets::kConsLen };
        if (!mem::ReadChain(g, offLen, 1, &len, sizeof(len), &fa)) return "err console not available";
        if (len == 0) return "ok (console log empty)";
        std::uintptr_t textPtr = 0;
        std::uintptr_t offText[1] = { offsets::kConsData };
        if (!mem::ReadChain(g, offText, 1, &textPtr, sizeof(textPtr), &fa) || !textPtr)
            return "err console buffer null";
        std::uint32_t want = (len > maxBytes) ? static_cast<std::uint32_t>(maxBytes) : len;
        std::vector<char> data(want);
        if (!mem::ReadChain(textPtr + (len - want), nullptr, 0, data.data(), want, &fa))
            return "err console read failed";
        return "ok " + OneLine(std::string(data.data(), data.size()));
    }
