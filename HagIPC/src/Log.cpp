...
// Documents/My Games/Skyrim Special Edition/SKSE — also where our config lives.
std::filesystem::path SkseUserDir() {
...
void Log::Init(const char* pluginName) {
    const auto file = (SkseUserDir() / (std::string(pluginName) + ".log")).string();
